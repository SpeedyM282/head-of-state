...
export function VectorScale({ vector }: { vector: number }) {
  ...
  return (
    <div className="panel p-3">
      ...
      <div className="relative h-3 w-full border border-(--paper-line)"
        style={{ background: 'linear-gradient(90deg, #7d9c6a 0%, #c5a75a 50%, #a8352c 100%)' }}>
        <div
          className="absolute top-[-5px] h-[22px] w-[3px] bg-(--text-ink)"
          style={{ left: `calc(${vector}% - 1px)`, transition: 'left 300ms' }}
        />
      </div>
      <div className="mt-1 flex justify-between text-[0.6rem] uppercase tracking-wider text-(--text-faint)">
