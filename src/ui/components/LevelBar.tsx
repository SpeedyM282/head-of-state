...
export function LevelBar({ label, value, inverted }: Props) {
  return (
    <div className="flex items-center gap-sp-2 py-sp-1">
      <span className="w-32 shrink-0 truncate text-label">{label}</span>
      ...
      <span className="num w-8 text-right text-num">{Math.round(value)}</span>
    </div>
  );
}
