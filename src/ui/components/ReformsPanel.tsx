...
import type { Effect, EffectTarget, Reform } from '../../core/types';
...
function targetLabel(target: EffectTarget, ui: Ui): string {
...
function nextReformId(reforms: Reform[], ownedReforms: string[]): string | null {
...
function LockIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" aria-hidden>
...
function EffectLines({ effects, ui }: { effects: Effect[]; ui: Ui }) {
  return (
    <ul className="mt-1">
      {effects.map((e, i) => {
        const color = e.target === 'vector' ? 'var(--gold)' : e.delta > 0 ? 'var(--ok)' : 'var(--stamp)';
        return (
          <li key={i} className="flex items-center justify-between gap-3 py-0.5 text-sm">
            <span className="text-[var(--text-faint)]">{targetLabel(e.target, ui)}</span>
            <span className="num" style={{ color }}>{signed(e.delta)}</span>
          </li>
        );
      })}
    </ul>
...
export function ReformsPanel({ onClose }: { onClose: () => void }) {
  ...
  const effectiveId = selectedId ?? nextReformId(branchReforms, state.ownedReforms);
  const selected = effectiveId ? branchReforms.find((r) => r.id === effectiveId) ?? null : null;
    ...
    return (
      <>
        <div className="flex-1 overflow-y-auto p-4">
          <p className="eyebrow">{ui.reformsPanel.tier} {r.tier + 1}</p>
          <h3 className="text-lg font-bold leading-tight">{loc(r.title, lang)}</h3>

          <p className="mt-2 text-sm leading-snug text-[var(--text-faint)]">{loc(r.description, lang)}</p>

          <p className="eyebrow mt-3">
            {ui.reformsPanel.influencePoints}: <span className="num text-[var(--gold)]">{r.costInfluence}</span>
            {' · '}{ui.reformsPanel.treasury}: <span className="num text-[var(--gold)]">{r.costTreasury}</span>
          </p>
        ...
        <div className="shrink-0 border-t border-[var(--paper-line)] p-4">
          {owned ? (
            ...
            <>
              ...
              {line && <p className="mt-1 text-center text-xs text-[var(--stamp)]">{line}</p>}
            </>
          )}
        </div>
      </>
  ...
  return (
    <div className="fixed inset-0 z-10 flex flex-col bg-[var(--ink)]">
      {/* Pinned header */}
      <header className="shrink-0 border-b border-[var(--paper-line)]">
        <div className="mx-auto flex max-w-[600px] items-center gap-3 px-3 py-2">
          <h2 className="text-lg font-bold">{ui.reformsPanel.title}</h2>
          <p className="eyebrow flex-1 truncate">
            {ui.reformsPanel.influencePoints}: <span className="num text-[var(--gold)]">{Math.floor(state.influence)}</span>
            {' · '}{ui.reformsPanel.treasury}: <span className="num text-[var(--gold)]">{Math.round(state.stats.treasury)}</span>
          </p>
          <button
            type="button"
            aria-label={ui.reformsPanel.close}
            onClick={onClose}
            className="-mr-1 flex h-8 w-8 shrink-0 items-center justify-center text-[var(--paper)]"
          >
            <CrossIcon />
          </button>
        </div>
      ...
      {/* Branch tabs */}
      <nav className="mx-auto flex w-full max-w-[600px] shrink-0 border-b border-[var(--paper-line)]" aria-label={ui.reformsPanel.title}>
        {BRANCHES.map((b) => {
      ...
      <div className="mx-auto flex w-full max-w-[600px] flex-1 flex-col overflow-hidden md:flex-row">
        <div className="min-h-0 flex-1 overflow-y-auto p-3">
          <div className="flex flex-col items-stretch">
            {branchReforms.map((r, i) => {
              ...
              return (
                <div key={r.id}>
                  {i > 0 && <div className="mx-auto h-4 w-0.5 bg-[var(--paper-line)]" />}
                  <button
                    type="button"
                    onClick={() => setSelectedId(r.id)}
                    className="panel w-full p-3 text-left"
                    style={{ opacity, boxShadow: isSelected ? '0 0 0 2px var(--gold)' : undefined }}
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {owned ? (
                        <span className="stamp shrink-0 text-[0.6rem]">{ui.reformsPanel.adopted}</span>
                      ) : locked ? (
                        <span className="shrink-0 text-[var(--text-faint)]" aria-label={ui.reformsPanel.locked}>
                          <LockIcon />
                        </span>
                      ) : (
                        <span className="num shrink-0 text-xs text-[var(--text-faint)]">
                          {r.costInfluence} {ui.reformsPanel.costInfluence} · {r.costTreasury} {ui.reformsPanel.costTreasury}
                        </span>
                      )}
                    </div>
                    <p className="mt-1 text-xs leading-snug text-[var(--text-faint)]">{loc(r.description, lang)}</p>
                  </button>
                </div>
              );
            })}
        ...
        {selected && (
          <aside className="flex max-h-[42vh] shrink-0 flex-col overflow-hidden rounded-t-2xl border-t border-[var(--paper-line)] bg-[var(--paper)] text-[var(--text-ink)] md:max-h-none md:w-80 md:rounded-none md:border-l md:border-t-0">
