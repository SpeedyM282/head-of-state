...
Player picks a country on a single whole-world map before the game starts — the fictional Absurdistan (list view only) or one of 168 real countries across fourteen click-to-zoom regions (Western Europe: 19, Eastern Europe: 21, North America: 16, South America: 12, Central Asia: 5, Middle East: 16, South & Southeast Asia: 17, Eastern Asia: 6, Oceania: 6, Northern Africa: 7, Western Africa: 15, Central Africa: 8, Eastern Africa: 15, Southern Africa: 5 — every sovereign/de-facto state per region with a shape in the vendored 110m topology; Russia/Turkey are selectable from Eastern Europe only). Europe's two sub-regions share one `europe.ts` content file, Asia's four share one `asia.ts` (same pattern as North/South America sharing `americas.ts`), and Africa's five share one `africa.ts` — only the map grouping is split, not the country data. At the default whole-world view, each region's countries are dissolved (via `topojson-client`'s `merge()`) into one borderless silhouette per region — individual country borders and click targets only appear after that region is entered. The map itself never changes projection — clicking a region animates `ZoomableGroup`'s pan/zoom to a precomputed frame over one fixed `geoEqualEarth` base; clicking a country within the active region opens its dossier. Each country has a real-world-derived profile (economy/corruption/democracy/development levels) that shapes the starting stats and governance vector. 7 stats (0-100): Economy, Treasury, Approval, Elite Loyalty, Stability, Development, Corruption. Corruption is the one INVERTED stat (higher is worse) — it skims treasury income, drags development, grows on its own (faster under totalitarianism), and above ~50 buys elite loyalty; it is never a defeat condition by itself. High Economy (>70) earns a prosperity bonus to income, which Corruption is the intended counterweight to. A Governance Vector scale (0-100: democracy → totalitarianism) shifts as a consequence of player choices and changes game rules per zone. Real-time, Plague Inc. style: time auto-advances (1 tick = 1 game month; normal 20s / fast 10s per month, plus pause) and the player buys reforms / answers events while the clock runs — no "next turn" button. Events and the open reforms panel auto-pause the clock (manual pause wins). Each tick → stats interact → win/lose checks. The clock lives in the store; `core` stays pure and turn-based. Play runs in 48-month terms with an election at the end of each (zone bends the result: democratic by a strict approval bar, authoritarian with «админресурс», totalitarian a 99.7% formality that costs reputation). Win the election → next term (inter-term inauguration screen; stats carry over); lose → 'elections' defeat. The constitution allows 2 terms — then amend it (open-ended play with per-term escalation so games still terminate) or step down. Stepping down is VICTORY, the canonical good ending («Ушёл непобеждённым»). 4 defeats: coup, revolution, default, lost elections. Tone: ironic satire, no real politicians/countries. First-time players get an optional tutorial (`ui/tutorial/`): a real Absurdistan/easy game narrated by the «Советник» advisor through 7 scripted, spotlight-anchored steps (clock paused until release), followed by one-time just-in-time tips triggered by game-state predicates (danger stat, vector-zone change, corruption>50, election approaching, unpaid elites) in every subsequent game. Presentation-only — `core`/`data/balance.ts` are untouched by it; tutorial progress persists under its own settings key (`prezident.tutorial.v1`), separate from the game save.
...
- New UI uses the fluid type/spacing token roles (`text-display/heading/body/label/caption/num`, `p-sp-*`/`gap-sp-*`/`m-sp-*` etc., see ARCHITECTURE.md §7a) — no raw `text-xs/sm/lg/xl/2xl` or bare `p-2/gap-3/m-4`-style numeric literals for these roles. Map by the role's meaning, not by matching a current pixel size. Touch targets (`min-h-11`/`min-h-9`) and one-off sub-token layout fixes are exempt, but any exception needs an inline comment explaining why the token doesn't fit.
